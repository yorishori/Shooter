...
# Loaded Scenes
var server_listener = load("res://network/ServerListener.tscn")
var server_advertiser = load("res://network/ServerAdvertiser.tscn")
var server_label = load("res://ui/labels/ServerLabel.tscn")
var player_label = load("res://ui/labels/PlayerLabel.tscn")
...
func _connected_to_server() -> void:
	...
	var player = {}
	player.network_id = get_tree().get_unique_id()
...
func _on_Join_btn_pressed():
	if userInput.text.strip_edges().length() >= 3:
		...
		listener_instance.connect("new_server",Callable(self,"_new_server_found"))
		listener_instance.connect("remove_server",Callable(self,"_server_removed"))

# Server AudioListener3D Signals
func _new_server_found(server_info):
	...
	instance.connect("join_pressed",Callable(self,"_server_join_pressed"))

func _server_join_pressed(ip) -> void:
	Network.ip = ip
	# Disable AudioListener3D
...
# Remote Functions
@rpc("any_peer", "call_local") func _update_player_list(new_players) -> void:
...
@rpc("any_peer") func _add_player(player) -> void:
	print("Is network server: " + str(get_tree().is_server()))
